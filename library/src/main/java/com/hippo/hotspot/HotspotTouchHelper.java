...
class HotspotTouchHelper implements View.OnTouchListener {

    private final List<Hotspotable> mReacterList = new ArrayList<>();
    ...
    public void addReacter(Hotspotable reacter){
        mReacterList.add(reacter);
    }

    public void removeReacter(Hotspotable reacter){
        mReacterList.remove(reacter);
    }

    public int getReacterCount() {
        return mReacterList.size();
    }
    ...
    private void setHotspot(View v, float x, float y) {
        ...
        for (Hotspotable owner : mReacterList) {
    ...
    private class UnsetPressedState implements Runnable {
        private final View mView;
